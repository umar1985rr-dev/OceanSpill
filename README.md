...
The segmentation model (U-Net with ResNet34 encoder) was trained on a public Kaggle dataset of **4,193** Sentinel-1 SAR image–mask pairs (3,354 train / 839 held-out test). Evaluation on the test set:

| Metric | Global (pixel-accumulated) | Per-image mean |
|--------|---------------------------|----------------|
| Dice Score | **0.871** | 0.744 |
| IoU | **0.771** | 0.635 |
| Precision | **0.847** | 0.716 |
| Recall | **0.897** | 0.833 |

Full results at `evaluation_results/model_metrics.json`. Reproduce with:
...
| **Python** | 3.10 to 3.12 (3.13+ may have PyTorch compatibility issues) |
| **Node.js** | 18+ (only needed for frontend development) |
| **Git** | Optional — for Git LFS model fallback only (primary: direct download) |
...
**First-time login:** The first account you create automatically becomes **admin**. After that, use the Login page (username + password) to access the platform. All other routes are protected — you'll be redirected to the login page if not authenticated.
...
| Source | Setup | Cost |
|--------|-------|------|
| **Sentinel Hub / Copernicus** | Free account at dataspace.copernicus.eu | Free |
...
**Satellite:** Use Copernicus / Sentinel Hub for live imagery.

**AIS:** Use AISHub for free live tracking, MarineTraffic for best coverage.
...
| `OCEANSPILL_SECRET_KEY` | (built-in) | JWT signing key — **set this in production** |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `FEED_SOURCE` | sentinel_hub | sentinel_hub |
...
- `feed_source` — Default: sentinel_hub
- `satellite_layer` — TRUE_COLOR or SAR
- `ais_source` — csv, aishub, or marinetraffic
...
|  └─ paper/                          # IEEE paper (LaTeX + markdown)
|
|-- Scripts --
|  └─ scripts/run_paper_evaluation.py # Reproducible model evaluation
|
|-- Evaluation --
|  ├─ evaluation_results/model_metrics.json
...
|        ├─ satellite_feed/          # Copernicus / Sentinel Hub
...
|     ├─ src/context/AuthContext.jsx  # JWT auth provider
|     ├─ src/pages/Login.jsx         # Login screen
|     └─ src/pages/                  # Dashboard, monitoring, map, etc.
|
|-- Data & Models --
...
| **`curl` download fails / "Access is denied"** | File permissions or directory missing | `start.bat` now auto-creates directory. Run as Administrator if persists, or manually create `models\fine_tuned\` folder |
| **`'git' is not recognized`** | Git not installed or not in PATH | Install Git from https://git-scm.com/ and CHECK "Git from the command line and also from 3rd-party software" |
| **`_pickle.UnpicklingError: invalid load key`** | Model file is Git LFS pointer (133 bytes) not actual model | `start.bat` now detects and re-downloads. Or manually download from GitHub Releases |
| **Python 3.13+ compatibility warnings** | PyTorch may not have wheels for newest Python | Install Python 3.10 - 3.12 from https://www.python.org/downloads/ |
...
- **Set `OCEANSPILL_SECRET_KEY`** to a random string in production — the default signing key is for development only. JWT tokens (24h access, 7d refresh) are used for authentication.

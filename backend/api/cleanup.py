...
# Cleanup recommendations recompute on every poll but the inputs only
# change when a new incident lands — reuse the result for a few seconds.
_cleanup_cache = TTLCache(ttl_seconds=10)
...
def recommend(
    ...
    # Cache key: all resolved scalar inputs (rounded to avoid noise).
    key = (
        round(spill_area, 4),
        round(distance_to_coast, 4),
        risk_level,
        round(wind_speed, 4),

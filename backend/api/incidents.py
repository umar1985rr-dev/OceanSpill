...
def get_incident(
    ...
    """Get a specific incident by its string id (INC-...) or numeric id."""
...
def _resolve_incident(db: Session, incident_id: str) -> Optional[Incident]:
    """Resolve by string id (INC-...) first, then by numeric id."""
    incident = db.query(Incident).filter(Incident.incident_id == incident_id).first()
    if incident:
        return incident
    if incident_id.isdigit():
        return db.query(Incident).filter(Incident.id == int(incident_id)).first()
    return None
